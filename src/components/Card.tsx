...
export const Card = ({ card, onClick, isAdmin, onImageEdit, href }: CardProps) => {
  ...
  if (href) {
    return (
      <Link 
        href={href}
        className="block bg-white/80 backdrop-blur-sm rounded-xl overflow-hidden shadow-sm border border-white/20 cursor-pointer hover:shadow-md transition-all group active:scale-[0.98] w-full h-full"
      >
        {CardContent}
      </Link>
    );
  ...
  return (
    <div
      onClick={onClick}
      className="bg-white/80 backdrop-blur-sm rounded-xl overflow-hidden shadow-sm border border-white/20 cursor-pointer hover:shadow-md transition-all group active:scale-[0.98] w-full h-full"
    >
      {CardContent}
    </div>
  );
